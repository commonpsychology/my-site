...
const POLL = {
  ...
  questions: [
    {
      id: 'q1',
      text: 'How often do you feel mentally overwhelmed in a typical week?',
      options: ['Rarely or never', 'Once or twice', '3–4 times', 'Almost every day'],
    },
    {
      id: 'q2',
      text: 'Have you ever sought professional mental health support?',
      options: ['Yes, currently', 'Yes, in the past', 'No, but I want to', 'No, I prefer self-help'],
    },
    {
      id: 'q3',
      text: 'What is the biggest barrier to accessing mental health care for you?',
      options: ['Cost / affordability', 'Stigma / judgment', 'Not knowing where to start', 'Time or availability'],
    },
    {
      id: 'q4',
      text: 'How do you primarily manage stress right now?',
      options: ['Exercise or movement', 'Talking to friends/family', 'Mindfulness / meditation', 'I struggle to manage it'],
    },
    {
      id: 'q5',
      text: 'How useful would a free mental health assessment be for you?',
      options: ['Very useful', 'Somewhat useful', 'Not sure', 'Not useful for me'],
    },
  ],
}

// Simulated vote counts (would come from backend later)
const MOCK_COUNTS = {
  q1: [120, 340, 210, 180],
  q2: [95,  270, 185, 140],
  q3: [310, 220, 190, 130],
...
export default function PollPopup({ onClose }) {
  ...
  // Results totals per question
  function getResults(qIdx, optIdx) {
    const qid = POLL.questions[qIdx].id
    const counts = [...MOCK_COUNTS[qid]]
    const myChoice = answers[qIdx]
    if (myChoice !== undefined) counts[myChoice] += 1
    const tot = counts.reduce((a, b) => a + b, 0)
    return { count: counts[optIdx], pct: Math.round((counts[optIdx] / tot) * 100) }
  }
      ...
      <div style={{
        ...
        <div style={{ padding: '1.5rem 1.75rem 1rem', borderBottom: '1.5px solid #000', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#000', marginBottom: 4 }}>
                🗳 Community Poll
              </div>
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', color: '#000', lineHeight: 1.25 }}>
                {POLL.title}
              </h2>
          ...
          {!submitted && (
            <div style={{ marginTop: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-body)', fontSize: '0.72rem', color: '#555', marginBottom: 5 }}>
              ...
              {POLL.questions.map((q, qi) => (
                <div key={q.id}>
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: '0.88rem', fontWeight: 700, color: '#000', marginBottom: '0.75rem', lineHeight: 1.4 }}>
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.72rem', fontWeight: 800, color: '#555', marginRight: 6 }}>{qi + 1}.</span>
                    {q.text}
                  </div>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            border: `1.5px solid ${selected ? '#000' : '#ccc'}`,
                            borderRadius: 8,
                        ...
                        >
                          ...
                          <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.83rem', fontWeight: selected ? 700 : 400, color: selected ? '#fff' : '#000' }}>{opt}</span>
                        </button>
                      )
                    })}
                  </div>
                  ...
                  <div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ flex: 1, height: 4, background: '#eee', borderRadius: 2, border: '1px solid #ccc', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: answers[qi] !== undefined ? '100%' : '0%', background: '#000', borderRadius: 2, transition: 'width 0.4s' }} />
                    </div>
                    <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.68rem', color: '#888', whiteSpace: 'nowrap' }}>
                      {answers[qi] !== undefined ? '✓ Answered' : 'Not answered'}
                    </span>
                  </div>
                </div>
              ))}
